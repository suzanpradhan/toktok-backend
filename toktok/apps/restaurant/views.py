    ...
    if request.method=="GET":
        ...
        for food in allFood:
            ...
            for add in food.addons.all():
                ...
                addons.append(addon)
            if food.cover_image:
                imageUrl=request.build_absolute_uri(food.cover_image.image.url)
            else:
                imageUrl=None
            data={
                'id': food.id,
...
def getAllCategories(request):
    if request.method=="GET":
        menuCollections=[]
        for menuCollection in MenuCollection.objects.all():
            if menuCollection.icon:
                imageUrl = request.build_absolute_uri(menuCollection.icon.image.url)
            else:
                imageUrl=None
            data={
                'name':menuCollection.name,
                'icon':imageUrl,
                'manager':menuCollection.manager.id,
            }
            menuCollections.append(data)

        return HttpResponse(json.dumps(menuCollections))
